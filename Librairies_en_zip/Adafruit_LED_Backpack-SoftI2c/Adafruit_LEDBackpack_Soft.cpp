...
void Adafruit_LEDBackpack_Soft::setBrightness(uint8_t b) {
  if (b > 15) b = 15;
  this->sWire.beginTransmission(i2c_addr);
  this->sWire.write(HT16K33_CMD_BRIGHTNESS | b);
  this->sWire.endTransmission();
}

void Adafruit_LEDBackpack_Soft::blinkRate(uint8_t b) {
  this->sWire.beginTransmission(i2c_addr);
  if (b > 3) b = 0; // turn off if not sure
  
  this->sWire.write(HT16K33_BLINK_CMD | HT16K33_BLINK_DISPLAYON | (b << 1));
  this->sWire.endTransmission();
}
...
void Adafruit_LEDBackpack_Soft::writeDisplay(void) {
  this->sWire.beginTransmission(i2c_addr);
  this->sWire.write((uint8_t)0x00); // start at address $00

  for (uint8_t i=0; i<8; i++) {
    this->sWire.write(displaybuffer[i] & 0xFF);
    this->sWire.write(displaybuffer[i] >> 8);
  }
  this->sWire.endTransmission();
}

void Adafruit_LEDBackpack_Soft::clear(void) {
  for (uint8_t i=0; i<8; i++) {
    displaybuffer[i] = 0;
  }
}
...
Adafruit_AlphaNum4::Adafruit_AlphaNum4(byte SDA, byte SCL): Adafruit_LEDBackpack_Soft(SDA, SCL) {
...
Adafruit_24bargraph::Adafruit_24bargraph(byte SDA, byte SCL): Adafruit_LEDBackpack_Soft(SDA, SCL) {
...
Adafruit_8x16matrix::Adafruit_8x16matrix(byte SDA, byte SCL): Adafruit_LEDBackpack_Soft(SDA, SCL),Adafruit_GFX(8, 16) {
}
...
Adafruit_8x16minimatrix::Adafruit_8x16minimatrix(byte SDA, byte SCL): Adafruit_LEDBackpack_Soft(SDA, SCL), Adafruit_GFX(8, 16) {
}
...
Adafruit_8x8matrix::Adafruit_8x8matrix(byte SDA, byte SCL): Adafruit_LEDBackpack_Soft(SDA, SCL),Adafruit_GFX(8, 8) {
}
...
Adafruit_BicolorMatrix::Adafruit_BicolorMatrix(byte SDA, byte SCL): Adafruit_LEDBackpack_Soft(SDA, SCL),Adafruit_GFX(8, 8) {
}
...
Adafruit_7segment::Adafruit_7segment(byte SDA, byte SCL): Adafruit_LEDBackpack_Soft(SDA, SCL) {
  position = 0;
}
...
void Adafruit_7segment::writeColon(void) {
    this->sWire.beginTransmission(i2c_addr);
    this->sWire.write((uint8_t)0x04); // start at address $02
    
    this->sWire.write(displaybuffer[2] & 0xFF);
    this->sWire.write(displaybuffer[2] >> 8);
