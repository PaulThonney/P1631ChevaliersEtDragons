...
#define PWM_MIN 50
#define PWM_MAX 200
#define PWM_AROUND 150

#define ADRESSE_ROUE 19
#define ADRESSE_SERVO 20

#define CODEUR1_PIN A1
#define CODEUR2_PIN A0


bool codeur_old[2] = {LOW, LOW};
int distanceRoue = 26; //distance entre les deur roues
int diametre = 6; // diamètre des roues
...
void setPWM() // Fonction appelée après l'attribution de setDirection
{
  ...
  switch (setDirection)
  {
    ...
    case TURN_RIGHT:
      ...
      pwm_left = PWM_MAX;
      pwm_right = round((float)angle / 90 * pwm_left);
      limitPWM();
...
void encodeur()
{
  float deltaDistance[3]; //Distances parcourues pour la roue droite(1), gauche(0) et totale(2)
  int codeur_new[2] = {digitalRead(CODEUR1_PIN), digitalRead(CODEUR2_PIN)};
  bool mouvement = LOW;
  float deltaAngle, deltaX, deltaY;
  //Serial.println(codeur_new[0]);
  for (int i = 0; i < 2; i++)
  {
    if (codeur_new[i] == HIGH && codeur_old[i] == LOW)
    {
      Serial.println("trou");
      deltaDistance[i] = PI * diametre / 20;
      mouvement = HIGH;
    }
    codeur_old[i] = codeur_new[i];
  }
  if (mouvement)
  {
    deltaDistance[2] = (deltaDistance[0] + deltaDistance[1]) / 2;
    deltaAngle = (deltaDistance[0] - deltaDistance[1]) / distanceRoue;
    deltaX = deltaDistance[2] * sin(positionAngle + deltaAngle / 2);
    deltaY = deltaDistance[2] * cos(positionAngle + deltaAngle / 2);
    positionAngle +=  deltaAngle;
    positionX +=  deltaX;
    positionY += deltaY;
    mouvement = LOW;
...
{
  int rayon = 300; //3 m
  int xMax = cos(positionAngle) * rayon;
  int yMax = sin(positionAngle) * rayon;
  if (positionX > xMax || positionY > yMax)
 {
      ...
      analogWrite(PWM_OUTPUT_MOTOR_L, PWM_MAX);
      analogWrite(PWM_OUTPUT_MOTOR_R, PWM_MAX);
 }
}
