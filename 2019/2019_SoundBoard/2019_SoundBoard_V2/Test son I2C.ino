...
void loop() {
  ...
  if (digitalRead (2) == 1) {
    sfx.playTrack((uint8_t)0); //Ici,receiveEvent le changement d'etat d'une pin déclkange la lecture d'une piste
  }
  else {
  }
  ...
  switch (piste) {
    ...
    case 4:
      sfx.playTrack((uint8_t)4);
      lecture = 0;
      break;

    case 5:
      sfx.playTrack((uint8_t)5);
      lecture = 0;
      break;

    case 6:
      sfx.playTrack((uint8_t)6);
      lecture = 0;
      break;

    case 7:
      sfx.playTrack((uint8_t)7);
      lecture = 0;
      break;
...
// La recupérationd Donnée sur un Bus I2C peut être transmis via un protocol sériel au module.


/*
  Le programme ci-dessous est l'exemple fournit pas Adafruit Industries. Il permet de Commander le système au travers d'un arduino via un port de communication seriel
...
{
  //Serial.println();
  piste = Wire.read(); //récuère l'addresse de l'émetteur
  lecture = 1;
}
